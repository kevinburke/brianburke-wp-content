			...
			if( $condition )
			{
				...
				if( $is_complex && !empty( $referer ) && !empty( $page ) )
				{
					if( $page != '*' )
					{
						...
						$pageid = $wpdb->get_var( 'SELECT `pageID` FROM `' . counterize_pageTable() . "` WHERE `url`='" . $wpdb->prepare( $page ) . "' LIMIT 1" );
					}

					//retrieve the refererID to decount
					$refererid = $wpdb->get_var( 'SELECT `refererID` FROM `' . counterize_refererTable() . "` WHERE `name`='" . $wpdb->prepare( $referer ) . "' LIMIT 1" );
	...
	if( ! empty( $agents_to_decount ) )
	{
		...
		foreach( $agents_to_decount as $id => $agent )
		{
			if( $agent['page'] != '*' )
			{
				...
				$sql_pages_1_1 .= $wpdb->prepare( $id );
				$sql_pages_1_2 .= '"' . $wpdb->prepare( $agent['page'] ). '"';
				$sql_pages_1_3 .= '"' . $wpdb->prepare( $agent['referer'] ). '"';

				$sql_logtable_1_1 .= $wpdb->prepare( $id );
				$sql_logtable_1_2 .= '"' . $wpdb->prepare( $agent['page'] ). '"';
				$sql_logtable_1_3 .= '"' . $wpdb->prepare( $agent['referer'] ). '"';
			}
			else
			{
				if( $nopage_cnt > 0)
				...
				$sql_pages_2_1 .= $wpdb->prepare( $id );
				$sql_pages_2_2 .= '"' . $wpdb->prepare( $agent['referer'] ). '"';

				$sql_logtable_2_1 .= $wpdb->prepare( $id );
				$sql_logtable_2_2 .= '"' . $wpdb->prepare( $agent['referer'] ). '"';
			}
		}
		$sql_pages_1_1 .= ' )'
					   . $sql_pages_1_2 . ' )'
