...
define( 'COUNTERIZE_VERSION_REV', 3  );
...
function counterize_getHitsOnCurrentArticle()
{
	...
	{
		$requesturl = $wpdb->prepare( $_SERVER['REQUEST_URI'] );
		$sql = "SELECT `count` "
...
function counterize_getentries_sql( $amount = 50, $entryID = null )
{
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " m.`IP` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} m.`IP` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
		}
		unset( $filters );
		$sql .= ") \n";
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " c.`code` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} c.`code` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
		}
		unset( $filters );
		$sql .= ") \n";
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " p.`url` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} p.`url` {$comparator} '" . $wpdb->prepare( $filter ) . "' ";
			}
		}
		unset( $filters );
		$sql .= ") \n";
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " r.`name` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} r.`name` {$comparator} '" . $wpdb->prepare( $filter ) . "' ";
			}
		}
		unset( $filters );
		$sql .= ") \n";
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " ua.`name` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} ua.`name` {$comparator} '" . $wpdb->prepare( $filter ) . "' ";
			}
		}
		unset( $filters );
		$sql .= ") \n";
	...
	)
	{
		...
		foreach( $filters as $filter )
		{
			...
			{
				$first = false;
				$sql .= " k.`keyword` {$comparator} '" . $wpdb->prepare( $filter ) ."' ";
			}
			else
			{
				$logical = 'OR';
				...
				$sql .= " {$logical} k.`keyword` {$comparator} '" . $wpdb->prepare( $filter ) . "' ";
			}
		}
		unset( $filters );
		$sql .= " ) \n";
	...
	)
	{
		...
		$where = TRUE;
		$sql .= $clause;
		$sql .= " m.`timestamp` BETWEEN '" . $wpdb->prepare( $datefilter1 ) . "' AND '" . $wpdb->prepare( $datefilter2 ) . "' ) \n";
	}
	...
	if( isset( $entryID ) )
	{
		$clause = $where === FALSE ? 'WHERE' : 'AND';
		$where = TRUE;
		$sql .= "{$clause} m.`id` = '" . $wpdb->prepare( $entryID ) . "' \n";
	}
