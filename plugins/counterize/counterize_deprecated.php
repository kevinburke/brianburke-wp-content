...
function counterize_most_visited_referrers( $number = 10, $width = 300 )
{
	...
	$sql = "SELECT `count` AS amount, `name` AS label, `name` AS label2, `name` AS url "
		...
		. " WHERE `name` <> 'unknown' "
		. " AND `name` NOT LIKE '" . $wpdb->prepare( get_option( "home" ) ) . "%%' "
		. " AND `name` NOT LIKE '" . $wpdb->prepare( get_option( "siteurl" ) ) . "%%' "
		. " ORDER BY amount DESC "
		. " LIMIT {$number}";
	$rows = $wpdb->get_results( $sql );
...
function counterize_most_visited_referrers24hrs( $number = 10, $width = 300 )
{
	...
	$sql = "SELECT COUNT(m.id) AS amount, r.name AS label, r.name AS label2, r.name AS url "
		...
		. " AND r.name <> 'unknown' "
		. " AND r.name NOT LIKE '" . $wpdb->prepare( get_option( "home" ) ) . "%%' "
		. " AND r.name NOT LIKE '" . $wpdb->prepare( get_option( "siteurl" ) ) . "%%' "
		. " AND m.timestamp >= '{$onedayago}'"
		. " GROUP BY r.name "
		. " ORDER BY amount DESC "
		. " LIMIT {$number}";
...
{
	global $wpdb;
	$geoip = $wpdb->prepare( counterize_get_option( 'geoip' ) );
	$number = $wpdb->prepare( $number );
...
function counterize_most_used_browsers_collapsible($nb_parent_items = 10, $nb_child_items = 15, $width = 300)
{
	...
	$nb_parent_items = (int) $wpdb->prepare( $nb_parent_items );
	$nb_child_items  = (int) $wpdb->prepare( $nb_child_items );
